  ...
  describe("Static Methods", () => {
    ...
    describe("Instance Methods", () => {
      ...
      describe("constructor", () => {
        it("should initialize with default values", () => {
          ...
          expect(adapter.functionCallingMode).toBe(
            GeminiAdapter.FUNCTION_CALLING_AUTO
          );
        });
      ...
      describe("countTokensAs", () => {
        it("should count tokens for generate request", async () => {
          ...
          const tokenCount = await adapter.countTokensAs(
            contentParts,
            "generate"
          );
      ...
      describe("Function calling", () => {
        describe("setFunctionCallingMode", () => {
          it("should set function calling mode", () => {
            ...
            expect(adapter.functionCallingMode).toBe(
              GeminiAdapter.FUNCTION_CALLING_NONE
            );
          });
          ...
          it("should update tool config when setting mode and functions exist", () => {
            ...
            expect(
              adapter.generationConfig.toolConfig.functionCallingConfig
            ).toBeDefined();
            expect(
              adapter.generationConfig.toolConfig.functionCallingConfig.mode
            ).toBe(GeminiAdapter.FUNCTION_CALLING_ANY);
          });
        ...
        describe("updateToolConfig", () => {
          ...
          it("should create toolConfig when functions exist", () => {
            ...
            expect(
              adapter.generationConfig.toolConfig.functionCallingConfig.mode
            ).toBe(adapter.functionCallingMode);
          });
          ...
          it("should preserve other toolConfig properties when removing functionCallingConfig", () => {
            ...
            expect(adapter.generationConfig.toolConfig.otherConfig).toEqual({
              value: "test",
            });
          });
        });
        ...
        describe("updateFunctionCallTool", () => {
          it("should remove function declarations when no functions are registered", () => {
            ...
            expect(
              adapter.generationConfig.tools[0].functionDeclarations
            ).toBeUndefined();
          });
          ...
          it("should create new function call tool when none exists", () => {
            ...
            expect(adapter.functionCallTool.functionDeclarations).toHaveLength(
              1
            );
          });
        ...
        describe("registerFunction", () => {
          ...
          it("should update existing function if procedure code already exists", () => {
            ...
            expect(Object.keys(adapter.functionRegistry)[0]).toBe(
              firstFunctionName
            );
            expect(
              adapter.functionRegistry[firstFunctionName].declaration
                .description
      ...
      describe("Content conversion", () => {
        describe("convertContentParts", () => {
          ...
          it("should convert object content parts", () => {
            const contentParts = [{ text: "Hello" }, { text: "World" }];
            const result = adapter.convertContentParts(contentParts);
    ...
    describe("Utility Functions", () => {
      describe("getTextFromResponse", () => {
        ...
        it("should use specified candidate index", () => {
          ...
          expect(adapter.getTextFromResponse(response, 1)).toBe(
            "Second candidate"
          );
        });
      });
    ...
    describe("Constants", () => {
      it("should export HarmCategory constants", () => {
        expect(HarmCategory.HARM_CATEGORY_HATE_SPEECH).toBe(
          "HARM_CATEGORY_HATE_SPEECH"
        );
        expect(HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT).toBe(
          "HARM_CATEGORY_SEXUALLY_EXPLICIT"
        );
        expect(HarmCategory.HARM_CATEGORY_HARASSMENT).toBe(
          "HARM_CATEGORY_HARASSMENT"
        );
        expect(HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT).toBe(
          "HARM_CATEGORY_DANGEROUS_CONTENT"
        );
      });

      it("should export HarmBlockThreshold constants", () => {
        expect(HarmBlockThreshold.BLOCK_LOW_AND_ABOVE).toBe(
          "BLOCK_LOW_AND_ABOVE"
        );
        expect(HarmBlockThreshold.BLOCK_MEDIUM_AND_ABOVE).toBe(
      ...
      it("should export EmbeddingTaskType constants", () => {
        expect(EmbeddingTaskType.SEMANTIC_SIMILARITY).toBe(
          "SEMANTIC_SIMILARITY"
        );
