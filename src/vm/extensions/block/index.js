...
class GeminiBlocks {
    ...
    getApiMenu () {
        const menu = [
            {
                text: formatMessage({
                    id: 'gai.apiMenu.gemini',
                    default: 'Gemini',
                    description: 'API menu item for Gemini'
                }),
                value: 'Gemini'
            },
            {
                text: formatMessage({
                    id: 'gai.apiMenu.openai',
                    default: 'OpenAI',
                    description: 'API menu item for OpenAI'
                }),
                value: 'OpenAI'
            }
        ];
    ...
    /**
     * Switch AI adapter based on API type
     * @param {string} apiType - 'OpenAI' or 'Gemini'
     */
    switchAdapter (apiType) {
        if (apiType === 'Gemini') {
            this.currentAdapterType = 'Gemini';
            this.AIAdapter = GeminiAdapter;
        } else {
            this.currentAdapterType = 'OpenAI';
            this.AIAdapter = OpenAIAdapter;
        }
    }
    ...
    partialResponseText (args, util) {
        ...
        return this.AIAdapter.getTextFromResponse(response);
    }
    ...
    _requestAI (prompt, isChat, util) {
        ...
        if (stackFrame.isResponseReceived) {
            if (this.allFunctionCallsFinished(stackFrame.functionCalls)) {
                ...
                return this.AIAdapter.getTextFromResponse(ai.getLastResponse());
            }
        }

        if (this.blockIsUsingInTarget('gai_whenPartialResponseReceived', target)) {
            const partialResponseHandler = partialResponse => {
                if (this.AIAdapter.getTextFromResponse(partialResponse) !== '') {
            ...
            if (!stackFrame.isRequesting) {
                ...
                request
                    .then(([response, functionCalls]) => {
                        ...
                        if (this.AIAdapter.getTextFromResponse(response) !== '') {
    ...
    responseText (args, util) {
        ...
        try {
            ...
            // the response is streaming
                ...
                responseText = this.AIAdapter.getTextFromResponse(response);
            } else {
                responseText = this.AIAdapter.getTextFromResponse(response, candidateIndex - 1);
            }
