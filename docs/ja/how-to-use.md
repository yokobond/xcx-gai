...
Browser LLM は、Hugging Face の ONNX モデルをブラウザ上で直接実行するAIプロバイダーです。外部APIキーが不要で、インターネット接続なしでもAI機能を利用できます（初回はモデルのダウンロードが必要です）。

### プロバイダーの選択

AIプロバイダーを選択するには ```AIに(プロバイダー)を使う``` ブロックを使い、```Browser LLM``` を選択します。

### モデルのダウンロード

Browser LLM を使用するには、事前にモデルをブラウザにダウンロードする必要があります。

```ブラウザLLMモデル (モデルID) を (モデルタイプ) としてダウンロード``` ブロックでダウンロードを行います。

- **[モデルID]** - Hugging Face 上のモデルID を指定します。末尾に `_dtype` を付けることでデータ型を指定できます（例: `onnx-community/gemma-4-E2B-it-ONNX_q4f16`）
  - データ型の指定がない場合、または `auto` を指定すると、モデルが対応するデータ型の中から自動的に最適なものが選ばれます
  - 利用可能なデータ型: `q4f16`（デフォルト）, `q4`, `q2f16`, `q2`, `q8`, `fp16`, `fp32`
- **[モデルタイプ]** - `テキスト生成` または `エンベディング` を選択します

ダウンロード中はプログレスバーで進捗が表示されます。モデルはブラウザのキャッシュに保存されるため、次回以降はダウンロードなしで利用できます。

ダウンロードせずにモデルを使用しようとした場合、「Browser LLM モデルがダウンロードされていません。先にダウンロードブロックを実行してください。」というエラーが表示されます。

### キャッシュのクリア

ダウンロード済みのモデルキャッシュを削除するには、```(番号) 番目のモデルのキャッシュをクリア``` ブロックを使います。```(番号)``` にはモデルの番号（1から開始）を指定します。

### モデルの動作

Browser LLM は実行環境に応じてデバイスを自動選択します:

1. **WebGPU** - GPU アクセラレーションを利用（高速）
2. **WASM** - WebGPU が使えない場合のフォールバック（互換性重視）

指定したデータ型がモデルで利用できない場合は、自動的に利用可能なデータ型にフォールバックします。

### デフォルトモデル

- テキスト生成: `onnx-community/gemma-4-E2B-it-ONNX`（`q4f16` 量子化）
- エンベディング: `onnx-community/all-MiniLM-L6-v2`

### 注意事項

- 初回使用時はモデルのダウンロードが必要です（モデルサイズによっては数百MBから数GB程度）
- WebGPU 対応ブラウザ（Chrome 113以降など）では高速に動作します
- 関数呼び出し（Function Calls）、構造化出力（Response Schema）などの一部機能はBrowser LLMでは利用できません
- ストリーミングレスポンス（```回答の一部を受け取ったとき```）は対応しています
